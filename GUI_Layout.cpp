...
namespace GUI{
    void RunGUI(){
        ...
        for (int i = 0; i <= rows; ++i) {
            ...
            ImVec2 lineEnd = ImVec2(calendarStartPos.x + daysInWeek * (cellSize.x + cellPadding), calendarStartPos.y + i * (cellSize.y + Padding));
        ...
        for (int i = 0; i <= daysInWeek; ++i) {
            ...
            ImVec2 lineEnd = ImVec2(calendarStartPos.x + i * (cellSize.x + Padding), calendarStartPos.y + float(rows) * (cellSize.y + VPadding));
            draw_list->AddLine(lineStart, lineEnd, IM_COL32(200, 200, 200, 255));
        }
        ...
        for (int day = 1; day <= daysInMonth; ++day) {
            if ((day + startDay - 1) % daysInWeek != 0) {
                ImGui::SameLine();
            } else if (day > 1) {
                ImGui::NewLine();
            }
            ImGui::Button(std::to_string(day).c_str(), cellSize);
        }
